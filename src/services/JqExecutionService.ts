...
import { JqBinaryService } from './JqBinaryService';
...
const spawnJq = (
  ...
  Effect.fn('JqExecutionService.spawnJq')(function* (input?: string) {
    return yield* Effect.async<string, JqExecutionError | InvalidJsonInputError>(
      (resume) => {
  ...
  '@jqpg/JqExecutionService',
  {
    effect: Effect.gen(function* () {
      ...
      const execute = Effect.fn('JqExecutionService.execute')(function* (
        ...
        return yield* spawnJq(jqPath, args, { cwd: options.cwd }, timeout)(
          input
        );
      });
